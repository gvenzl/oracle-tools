...
6) Optional: Move `login.sql` to the `$SQLPATH` folder

## Files

| File                     | Description                  |
|--------------------------|------------------------------|
| [login.sql](./login.sql) | SQL script that sets the SQLcl environment. Can either be the the current working directory or in `$SQLPATH`. |
